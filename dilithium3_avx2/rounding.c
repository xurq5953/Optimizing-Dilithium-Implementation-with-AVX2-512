...
**************************************************/
unsigned int make_hint_avx2(uint8_t hint[256], const __m256i *restrict a0, const __m256i *restrict a1) {
    unsigned int i, n = 0;
    __m256i f0, f1, g0, g1;
    uint32_t bad;
    uint64_t idx;
    ...
    for (i = 0; i < N / 8; ++i) {
        ...
        memcpy(&idx, DILITHIUM3_AVX2_idxlut[bad], 8);
...
*
**************************************************/
void use_hint_avx2(__m256i *b, const __m256i *a, const __m256i *restrict hint) {
    unsigned int i;
    __m256i a0[N / 8];
    __m256i f, g, h, t;
    ...
    DILITHIUM3_AVX2_decompose_avx(b, a0, a);
    for (i = 0; i < N / 8; i++) {
